...
class GaussianDiffusion:
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        Compute the mean and variance of the diffusion posterior:
            q(x_{t-1} | x_t, x_0)
        """
        # print(f"x_start.shape: {x_start.shape} | x_t.shape: {x_t.shape}")
        # print(f"x_start: {x_start.shape}, x_t: {x_t.shape}, t: {t}")
    ...
    def p_mean_variance(
        ...
        C=1
        assert t.shape == (B,)
        # print(f"WHAT IS MODEL? : {model}")
        model_output = model(x, self._scale_timesteps(t), **model_kwargs)
        # print("It should at least get up to here!")
        ...
        # print(f"self.model_mean_type: {self.model_mean_type}")
        # print(f"self.model_var_type: {self.model_var_type}")
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                ...
                # print(f"pred_xstart: {pred_xstart.shape}")
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        ...
        # print("Does it get here every time?")
        return (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t
            - _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape) * eps
            # - _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape) * eps[:,1:,:,:]
        )
    ...
    def p_sample(
        ...
        # print(f"output of p_mean_variance: {out.shape}")
    ...
    def p_sample_loop_known(
        ...
        img=img.to(device)
        # print(f"img shape in p_sample_loop_known: {img.shape}")
        # print(f"x_noisy shape in p_sample_loop_known: {x_noisy.shape}")
        ...
        if progress:
           ...
           for i in indices:
                ...
                #     print('sampling step', i)
                    ...
                    # viz.image(visualize(img.cpu()[0, 0,...]), opts=dict(caption="pet1_"+ str(i) ))
                    # viz.image(visualize(img.cpu()[0, 1,...]), opts=dict(caption="pet2_"+ str(i) ))
                    # viz.image(visualize(img.cpu()[0, 2,...]), opts=dict(caption="pet3_"+ str(i) ))
                    # viz.image(visualize(img.cpu()[0, 3,...]), opts=dict(caption="pet4_"+ str(i) ))

                with th.no_grad():
                    # if img.shape != (1, 5, 224, 224):
                    # print("Fucking here!")
                    # print(f" HERE img.shape: {img.shape}")
                    ...
                    yield out
                    img = out["sample"]
                    # print(f"out['sample'] size: {img.shape}")
