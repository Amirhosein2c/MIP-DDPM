...
from pytomography.utils import get_blank_below_above, compute_TEW, get_mu_from_spectrum_interp, compute_TEW_Zero_Upper_Scatter
...
def get_angles_sorting_indices(ds):
    flags = {'multi_energy_window': False, 'multi_time_slot': False}
...
def get_scatter_from_TEW_new(file, projections_all, index_peak, index_lower, index_upper):
    """Gets an estimate of scatter projection data from a DICOM file using the triple energy window method.
...
def get_scatter_from_TEW_Zero_Upper_Scatter(file, projections_all, index_peak, index_lower, index_upper):
    """Gets an estimate of scatter projection data from a DICOM file using the triple energy window method.
    ...
    ww_upper = get_window_width(ds, index_upper)
    # projections_all = get_projections(file)
    scatter = compute_TEW_Zero_Upper_Scatter(projections_all[index_lower],projections_all[index_upper], ww_lower, ww_upper, ww_peak)
    return scatter.to(pytomography.device).unsqueeze(0)
...
def save_dcm(
    ...
    _series_description = ds.SeriesDescription
    series_description = _series_description.split("WB SPECT CT")[0]
    series_description = f"{ds.PatientID} {series_description} WB SPECT CT {recon_name.split('-')[-1]}"
    ds.SeriesDescription = series_description
    print(f"ds SeriesDescription: {ds.SeriesDescription}")
    # Create all slices
